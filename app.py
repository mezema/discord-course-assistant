...
stub = modal.Stub(
    name="discord-course-assistant",
    image=image,
    secrets=[
        # this is where we add API keys, passwords, and URLs, which are stored on Modal
        modal.Secret.from_name("mongodb-larn"),
        modal.Secret.from_name("openai-api-key-larn"),
    ],
    mounts=[
...
@modal.web_endpoint(method="GET", label="discord-course-assistant-hook")
def web(query: str, request_id=None):
    ...
    answer = qanda_langchain(query, request_id=request_id, with_logging=False)
...
@modal.asgi_app(label="discord-course-assistant")
def fastapi_app():
    ...
    def chain_with_logging(*args, **kwargs):
        return qanda_langchain(*args, with_logging=False, **kwargs)
